...
import org.fog.utils.FogLinearWithSnapPowerModel;
...
public class VRGameFog {
	/**
	 * cloud, proxy and mobile are all modeled as fogDevices
	 */
	static List<FogDevice> fogDevices = new ArrayList<>();
	/**
	 * every dep has their own sensor and actuator
	 */
...
//			moduleMapping.addModuleToDevice("connector", "cloud"); // fixing all instances of the Connector module to the Cloud
			for(FogDevice fog: fogDevices){
				if(fog.getName().startsWith("_mobile")){
				...
				if(fog.getName().startsWith("mobile")){
					moduleMapping.addModuleToDevice("client", fog.getName()); // fixing all instances of the Connector module to the Cloud
					moduleMapping.addModuleToDevice("concentration_calculator", fog.getName()); // fixing all instances of the Connector module to the Cloud
				}
			...
			controller.submitApplication(application, 0,
							(true) ? (new ModulePlacementMapping(fogDevices, application, moduleMapping))
	...
	private static FogDevice addMobile(String id, int userId, String appId, int parentId) {
		...
		Sensor eegSensor = new Sensor("sensor-" + id, "EEG", userId, appId, new NormalDistribution(Configs.MEAN,Configs.STD_DEV)); // inter-transmission time of EEG sensor follows a deterministic distribution
	...
	private static FogDevice createFogDevice(String nodeName, long mips,
		...
		int bw = 10000;
		StreamOperatorScheduler streamOperatorScheduler = new StreamOperatorScheduler(peList);
		streamOperatorScheduler.name = nodeName;
		PowerHost host = new PowerHost(
						...
						storage,
						peList,
						streamOperatorScheduler,
						new FogLinearWithSnapPowerModel(nodeName, busyPower, idlePower, sleepPower)
		);
...
//		System.err.println(fogdevice.getName() + "    " + fogdevice.getId());
		return fogdevice;
	}
...
//		application.addAppModule("connector", 10); // adding module Connector to the application model
		...
		application.addAppEdge("client", "concentration_calculator", 4800, 500, "_SENSOR", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
...
//		application.addAppEdge("client", "DISPLAY", 1000, 500, "SELF_STATE_UPDATE", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE
		application.addAppEdge("concentration_calculator", "DISPLAY", 1000, 500, "CONCENTRATION", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type GLOBAL_STATE_UPDATE
